...
1. Run the stage (Hamilton Driver execution) and produce explicit outputs.
...
7. If approved, write `review/<stage>/approval.json` and only then update the narrative.
8. If rejected, write `review/<stage>/feedback.json`, incorporate feedback, re-run.
...
This keeps data acquisition separate from the Hamilton computation layer.
...
- `review/<stage>/approval.json` — approved interpretation text
